...
PARENTDIR = xconstants.WORD_ZEROSHOT_BASEPATH/"dev"
TESTDIR = xconstants.WORD_ZEROSHOT_BASEPATH/"test"

PRETRAINED_DIR = xconstants.WORD_PROPOSED_BASEPATH / "dev"
...
experiment_list = [
    ...
    #                      checkpoint=get_best_checkpoint(get_experiment_dir(PRETRAINED_DIR, "xanewBE"))),
    ...
    #                      checkpoint=get_best_checkpoint(get_experiment_dir(PRETRAINED_DIR, "xanewVA"))),
    ...
    Checkpoint_Test_Word(name="kapucuBE", parent_dir=PARENTDIR, dataset_class=data.words.Kapucu_BE,
                         split="dev", embeddings=data.vectors.Facebook_CommonCrawl_Turkish, embedding_limit=EMBEDDING_LIMIT,
                         model=Word_Proposed.get_model()[0].select(encoder="words", decoders="be5"),
